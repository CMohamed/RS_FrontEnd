var lyrNumber = 3;  // anrdoha 0 ila kan app atbda o ta layer makainach
...
function removeLayer(element){
    console.log(element);
    var oldId = element.parentElement.parentElement.id;

    console.log(oldId);


    var indexS = oldId.substring(3);
    var index = parseInt(indexS);

    var layer = element.parentElement.parentElement;
    layer.parentNode.removeChild(layer);
    for(var i=index+1;i<lyrNumber;i++){
        var leftLyr = document.getElementById("lyr"+ i.toString());
        leftLyr.id = "lyr" + (i-1).toString();
    }
    lyrNumber --;
}
function showLayer(){
...
*/
    ...
    id="lyr" + lyrNumber.toString() ;
    lyrNumber++;
    var html = '<li id =" '+ id +'"  class="collection-item" name="'+name+'">' +
        '        <div >' +name+
        ...
        '            <i style="cursor:pointer" onclick="removeLayer()" class="material-icons secondary-content">delete</i>\n' +
